...
try {
    ...
    $checkQuery = "SELECT id, name FROM products WHERE id = ?";
    $checkStmt = $db->prepare($checkQuery);
    $checkStmt->execute([$product_id]);
    $existing_product = $checkStmt->fetch(PDO::FETCH_ASSOC);
    ...
    for ($colorIndex = 0; $colorIndex < count($_POST['color_names'] ?? []); $colorIndex++) {
        $colorImageFieldName = "color_images_" . $colorIndex;
