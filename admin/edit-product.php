...
$page_title = $is_edit_mode ? "Edit Product - MaterialM Admin" : "Add New Product - MaterialM Admin";
...
if ($is_edit_mode) {
    ...
    $query = "SELECT color_name, color_code FROM product_colors WHERE product_id = ? ORDER BY sort_order";
    $stmt = $db->prepare($query);
    $stmt->execute([$product_id]);
    $product_colors = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch product sizes
    $query = "SELECT size_name, size_value FROM product_sizes WHERE product_id = ? ORDER BY sort_order";
    $stmt = $db->prepare($query);
    $stmt->execute([$product_id]);
    $product_sizes = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch product images by color
    $query = "SELECT pi.*, pc.color_name, pc.color_code 
    ...
    foreach ($product_images as $image) {
        $color_index = array_search($image['color_name'], array_column($product_colors, 'color_name'));
        if ($color_index !== false) {
            $color_images[$color_index][] = [
                'name' => basename($image['image_path']),
                'path' => '../store/' . $image['image_path'],
    ...
    $form_data = [
        ...
        'short_description' => $existing_product['short_description'],
        'price' => $existing_product['price'],
        'discounted_price' => $existing_product['discounted_price'],
        'custom_name_enabled' => $existing_product['custom_name_enabled'],
        'pouch_custom_price' => $existing_product['pouch_custom_price'],
        'sajadah_custom_price' => $existing_product['sajadah_custom_price'],
        'status' => $existing_product['status'],
        'featured' => $existing_product['featured'],
        'categories' => $product_categories,
        'color_names' => array_column($product_colors, 'color_name'),
        'color_codes' => array_column($product_colors, 'color_code'),
        'color_images' => $color_images,
        'size_names' => array_column($product_sizes, 'size_name'),
        'size_values' => array_column($product_sizes, 'size_value')
    ];
}

// Override with session data if available (for error repopulation)
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-semibold mb-1"><?php echo $is_edit_mode ? 'Edit Product' : 'Add New Product'; ?></h4>
            <p class="text-muted mb-0"><?php echo $is_edit_mode ? 'Update product information' : 'Add a new product to your catalog'; ?></p>
        </div>
        <div>
            <?php if (!$is_edit_mode): ?>
    ...
    <form id="productForm" action="<?php echo $is_edit_mode ? 'edit-product-handler.php' : 'add-product-handler.php'; ?>" method="POST" enctype="multipart/form-data">
        <?php if ($is_edit_mode): ?>
        ...
        <div class="d-flex justify-content-end gap-2 mb-4">
            ...
            <button type="submit" class="btn btn-primary">
                ...
                <?php echo $is_edit_mode ? 'Update Product' : 'Save Product'; ?>
            </button>
        </div>
    </form>
</div>
...
<?php if (!$is_edit_mode): ?>
// Generate default values for all form fields (only for add mode)
